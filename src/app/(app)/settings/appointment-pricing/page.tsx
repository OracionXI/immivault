...
export default function AppointmentPricingPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setDialogOpen(false);
      setForm({ appointmentType: "", priceInCents: "", currency: "USD", description: "", isActive: true });
    } catch (err) {
    ...
    <RoleGuard allowedRoles={["admin"]} redirectTo="/settings">
      <div className="space-y-6">
        <PageHeader
          ...
          description="Set prices for each appointment type shown in the client portal."
        />
                ...
                {pricing.map((p) => (
                  <div key={p._id} className="flex items-center justify-between p-3 rounded-lg border bg-card">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-semibold text-sm">{formatPrice(p.priceInCents, p.currency)}</span>
                      <Button
